...
# Менеджеры пакетов
npm i -g grexcode-ai@latest        # или bun/pnpm/yarn
scoop install grexcode             # Windows
choco install grexcode             # Windows
brew install grexlabs/tap/grexcode # macOS и Linux (рекомендуем, всегда актуально)
brew install grexcode              # macOS и Linux (официальная формула brew, обновляется реже)
sudo pacman -S grexcode            # Arch Linux (Stable)
paru -S grexcode-bin               # Arch Linux (Latest from AUR)
mise use -g grexcode               # любая ОС
nix run nixpkgs#grexcode           # или github:grexlabs/grexcode для самой свежей ветки dev
```
...
GrexCode также доступен как десктопное приложение. Скачайте его со [страницы релизов](https://github.com/grexlabs/grexcode/releases) или с [grexlabs.in/download](https://grexlabs.in/download).
...
4. `$HOME/.grexcode/bin` - Fallback по умолчанию
...
В GrexCode есть два встроенных агента, между которыми можно переключаться клавишей `Tab`.
...
Подробнее об [agents](https://grexlabs.in/docs/agents).

### Документация

Больше информации о том, как настроить GrexCode: [**наши docs**](https://grexlabs.in/docs).

### Вклад

Если вы хотите внести вклад в GrexCode, прочитайте [contributing docs](./CONTRIBUTING.md) перед тем, как отправлять pull request.

### Разработка на базе GrexCode

Если вы делаете проект, связанный с GrexCode, и используете "grexcode" как часть имени (например, "grexcode-dashboard" или "grexcode-mobile"), добавьте примечание в README, чтобы уточнить, что проект не создан командой GrexCode и не аффилирован с нами.

---

**Присоединяйтесь к нашему сообществу** [Discord](https://discord.gg/grexcode) | [X.com](https://x.com/grexcode)
