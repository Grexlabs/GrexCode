...
function headers(contentType: string) {
  return { "Content-Type": contentType, "User-Agent": `grexcode/${InstallationVersion}` }
}
...
const successPage =
  "<!doctype html><title>GrexCode</title><h1>Authorization successful</h1><p>You can close this window.</p>"
const errorPage = (message: string) =>
  `<!doctype html><title>GrexCode</title><h1>Authorization failed</h1><p>${message.replace(/[&<>"']/g, "")}</p>`
