...
import { ToolFailure } from "@grexlabs/llm"
...
export class ConfigService extends Context.Service<ConfigService, Config>()("@grexcode/v2/WebSearchConfig") {}

/** Isolates the retained product environment contract from the generic tool implementation. */
export const defaultConfigLayer = Layer.sync(ConfigService, () =>
  ConfigService.of({
    provider:
      process.env.GREXCODE_WEBSEARCH_PROVIDER === "exa" || process.env.GREXCODE_WEBSEARCH_PROVIDER === "parallel"
        ? process.env.GREXCODE_WEBSEARCH_PROVIDER
        : undefined,
    enableExa: truthy("GREXCODE_EXPERIMENTAL") || truthy("GREXCODE_ENABLE_EXA") || truthy("GREXCODE_EXPERIMENTAL_EXA"),
    enableParallel: truthy("GREXCODE_ENABLE_PARALLEL") || truthy("GREXCODE_EXPERIMENTAL_PARALLEL"),
