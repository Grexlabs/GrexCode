...
import { ModelsDevPlugin } from "@grexlabs/core/plugin/models-dev"
...
describe("ModelsDevPlugin", () => {
  it.effect("registers key methods for providers with environment variables", () =>
    Effect.acquireUseRelease(
      Effect.sync(() => {
        const previous = {
          path: Flag.GREXCODE_MODELS_PATH,
          disabled: Flag.GREXCODE_DISABLE_MODELS_FETCH,
        }
        Flag.GREXCODE_MODELS_PATH = path.join(import.meta.dir, "fixtures", "models-dev.json")
      ...
      (previous) =>
        Effect.sync(() => {
          Flag.GREXCODE_MODELS_PATH = previous.path
          Flag.GREXCODE_DISABLE_MODELS_FETCH = previous.disabled
        }),
    ),
  )
})
