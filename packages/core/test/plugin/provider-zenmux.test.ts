...
import { ProviderPlugins } from "@grexlabs/core/plugin/provider"
import { ZenmuxPlugin } from "@grexlabs/core/plugin/provider/zenmux"
...
describe("ZenmuxPlugin", () => {
  ...
  it.effect("applies the exact legacy Zenmux headers", () =>
    Effect.gen(function* () {
      ...
      expect(result.request.headers).toEqual({ "HTTP-Referer": "https://grexlabs.in/", "X-Title": "grexcode" })
