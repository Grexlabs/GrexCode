...
describe("Npm.sanitize", () => {
  test("keeps normal scoped package specs unchanged", () => {
    expect(Npm.sanitize("@grexcode/acme")).toBe("@grexcode/acme")
    expect(Npm.sanitize("@grexcode/acme@1.0.0")).toBe("@grexcode/acme@1.0.0")
    expect(Npm.sanitize("prettier")).toBe("prettier")
  })

  test("handles git https specs", () => {
    const spec = "acme@git+https://github.com/grexcode/acme.git"
    const expected = win ? "acme@git+https_//github.com/grexcode/acme.git" : spec
