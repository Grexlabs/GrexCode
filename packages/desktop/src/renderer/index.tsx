...
import type { UpdaterState } from "@grexlabs/app/updater"
...
import "./styles.css"
import { Splash } from "@grexlabs/ui/logo"
    ...
    integrations: (integrations) => {
      return integrations.filter(
        (i) =>
          i.name !== "Breadcrumbs" &&
          !(
            import.meta.env.GREXCODE_CHANNEL === "prod" &&
...
const deepLinkEvent = "grexcode:deep-link"

const emitDeepLinks = (urls: string[]) => {
  if (urls.length === 0) return
  window.__GREXCODE__ ??= {}
  const pending = window.__GREXCODE__.deepLinks ?? []
  window.__GREXCODE__.deepLinks = [...pending, ...urls]
...
render(() => {
  ...
  const loadLocale = async () => {
    const current = await platform.storage?.("grexcode.global.dat").getItem("language")
