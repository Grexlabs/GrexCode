...
export function createWslServersController(
  ...
  const checkOpencode = async (distro: string, opts?: { signal?: AbortSignal }) => {
    ...
    const version = resolved
      ...
      : null
    return grexcodeCheck(distro, resolved, version, appVersion)
  }
  ...
  const refreshOpencodeCheckBackground = (id: string, distro: string) => {
    void checkOpencode(distro)
      ...
      .catch((error) => {
        ...
        logger?.error("wsl grexcode check failed", { id, distro, message })
      })
  }
    ...
    await Promise.all(
      state.servers.map((item) =>
        checkOpencode(item.config.distro)
          ...
          .catch((error) => {
            const message = error instanceof Error ? error.message : String(error)
            logger?.error("wsl grexcode check failed", {
              id: item.config.id,
  ...
  return {
    ...
    async probeOpencode(name: string) {
      await runJob({ kind: "probe-grexcode", distro: name, startedAt: Date.now() }, async (abort) => {
    ...
    async installOpencode(name: string) {
      await runJob({ kind: "install-grexcode", distro: name, startedAt: Date.now() }, async (abort) => {
        ...
        await refreshOpencodeCheck(name, { signal: abort.signal })
        expectOpencodeVersion(state.grexcodeChecks[name]?.version ?? null, appVersion, name)
    ...
    async removeServer(id: string) {
      ...
      setState({
        ...
        ...(distro ? clearWslDistroState(state.distroProbes, state.grexcodeChecks, distro) : {}),
      })
    },
...
function grexcodeCheck(
  ...
  if (!resolvedPath) {
    return {
      ...
      expectedVersion,
      matchesDesktop: null,
      error: "grexcode is not installed in this distro",
    }
  }
  if (!version) {
    return {
      ...
      expectedVersion,
      matchesDesktop: null,
      error: "grexcode is installed but could not run",
    }
  }
  return {
    distro,
