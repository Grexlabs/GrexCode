...
export async function spawnWslSidecar(
  ...
  const grexcode = await resolveWslOpencode(distro)
  if (!grexcode) throw new Error(`GrexCode is not installed in ${distro}`)

  const port = await allocatePort()
  const password = randomUUID()
  const username = "grexcode"
  const script = [
    "set -euo pipefail",
    'cd "$HOME" || cd /',
    ...
    "export PATH",
    "export WSLENV=",
    "export GREXCODE_EXPERIMENTAL_DISABLE_FILEWATCHER=true",
    "export GREXCODE_CLIENT=desktop",
    `export GREXCODE_SERVER_USERNAME=${shellEscape(username)}`,
    `export GREXCODE_SERVER_PASSWORD=${shellEscape(password)}`,
    'export XDG_STATE_HOME="$HOME/.local/state"',
    `exec ${shellEscape(grexcode)} --print-logs --log-level ${app.isPackaged ? "WARN" : "INFO"} serve --hostname 0.0.0.0 --port ${port}`,
  ].join("\n")
