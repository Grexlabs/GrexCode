...
function tauriAppId() {
  return app.isPackaged ? TAURI_APP_IDS[CHANNEL] : "ai.grexcode.desktop.dev"
}

// Migrate a single Tauri .dat file into the corresponding electron-store.
// `grexcode.settings.dat` is special: it maps to the `grexcode.settings` store
...
// renderer already passes via IPC (e.g. `"default.dat"`, `"grexcode.global.dat"`).
function migrateFile(datPath: string, filename: string) {
  ...
  // grexcode.settings.dat → the electron settings store ("grexcode.settings").
  // All other .dat files keep their full filename as the store name so they match
  // what the renderer passes via IPC (e.g. "default.dat", "grexcode.global.dat").
  const storeName = filename === "grexcode.settings.dat" ? "grexcode.settings" : filename
