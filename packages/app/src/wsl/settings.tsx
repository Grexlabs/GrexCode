    ...
    <Show when={api}>
      <For each={props.servers()}>
        {(item) => {
          ...
          const check = () => wsl.data?.grexcodeChecks[item.config.distro]
          const grexcodeAction = () => wslOpencodeAction(check())
          const busy = () => wsl.data?.job?.kind === "install-grexcode" && wsl.data.job.distro === item.config.distro
          return (
