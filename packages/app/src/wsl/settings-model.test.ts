...
describe("WSL server settings presentation", () => {
  ...
  test("offers install and update only when GrexCode needs attention", () => {
    ...
    ).toBe("Install GrexCode")
    expect(
      wslOpencodeAction({
        distro: "Debian",
        resolvedPath: "/usr/local/bin/grexcode",
        version: "1.2.2",
        expectedVersion: "1.2.3",
        matchesDesktop: false,
        error: null,
      }),
    ).toBe("Update GrexCode")
    expect(
      wslOpencodeAction({
        distro: "Debian",
        resolvedPath: "/usr/local/bin/grexcode",
  ...
  test("probes the selected distro before entering the GrexCode step", async () => {
    const calls: string[] = []
    ...
    expect(calls).toEqual(["Debian", "grexcode"])
  })
})
