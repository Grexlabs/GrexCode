...
import { useDialog } from "@grexlabs/ui/context/dialog"
import { Spinner } from "@grexlabs/ui/spinner"
import { showToast } from "@grexlabs/ui/toast"
...
type WslServerStep = "wsl" | "distro" | "grexcode"

const STEPS: WslServerStep[] = ["wsl", "distro", "grexcode"]
...
export function DialogAddWslServer(props: DialogWslServerProps = {}) {
  ...
  const grexcodeCheck = createMemo(() => {
    const distro = selectedDistro()
    if (!distro) return null
    return current()?.grexcodeChecks[distro] ?? null
  })
  ...
  const grexcodeReady = createMemo(() => {
    const check = grexcodeCheck()
  ...
  const installingOpencode = createMemo(() => {
    ...
    return job?.kind === "install-grexcode" && job.distro === selectedDistro()
  })
  const allReady = createMemo(() => wslReady() && distroReady() && grexcodeReady())
  const addDisabled = createMemo(() => {
    const job = current()?.job
    if (!job) return store.adding
    return store.adding || job.kind !== "probe-grexcode"
  })
  ...
  const grexcodeMessage = createMemo(() => {
    ...
    if (grexcodeReady()) {
      return distro
        ? language.t("wsl.onboarding.grexcodeReadyIn", { distro })
        ...
        title:
          step === "wsl"
            ...
            : step === "distro"
              ...
              : language.t("wsl.onboarding.step.grexcode"),
        state:
          active === step
            ? "current"
            : step === "wsl"
              ...
              : step === "distro"
                ? distroReady()
                  ...
                  : index > activeIndex
                    ? "locked"
                    : "warning"
                : grexcodeCheck()?.matchesDesktop === false
                  ? "warning"
                  : grexcodeReady()
                    ? "done"
                    : index > activeIndex
      ...
      >
        ...
        >
          ...
          <Switch>
            ...
            <Match when={activeStep() === "grexcode"}>
              <div class="rounded-md bg-surface-base p-4 flex flex-col gap-3">
                <div class="flex items-center justify-between gap-3">
                  <div class="text-14-medium text-text-strong">{language.t("wsl.onboarding.step.grexcode")}</div>
                  <div class="flex items-center gap-2">
                    <Show when={selectedDistro()}>
                      <Button
                        variant="ghost"
                    ...
                    <Show when={!grexcodeReady() || grexcodeCheck()?.matchesDesktop === false}>
                      <Button
                        variant="secondary"
                        size="large"
                        disabled={busy()}
                ...
                <div class="text-12-regular text-text-weak whitespace-pre-wrap break-words">{grexcodeMessage()}</div>
                <Show when={grexcodeCheck()?.matchesDesktop === false ? grexcodeCheck() : null}>
                  {(check) => (
                    <div class="rounded-md border border-border-weak-base px-3 py-3 flex flex-col gap-1">
                      <div class="text-12-regular text-text-weak">
          ...
          <Show when={activeStep() === "grexcode" && allReady() && selectedDistro()}>
