...
export type WslJob =
  ...
  | { kind: "probe-grexcode"; distro: string; startedAt: number }
  | { kind: "install-grexcode"; distro: string; startedAt: number }
...
export type WslServersPlatform = {
  ...
  probeGrexcode(name: string): Promise<void>
  installGrexcode(name: string): Promise<void>
