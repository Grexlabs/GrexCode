  ...
  test("moves an existing canonical web bucket into local scope", () => {
    expect(
      migrateCanonicalLocalServerState(
        {
          list: [],
          projects: { "https://grexcode.example.com": [{ worktree: "/remote", expanded: true }] },
          lastProject: { "https://grexcode.example.com": "/remote" },
        },
        ServerConnection.Key.make("https://grexcode.example.com"),
      ),
    ).toEqual({
      list: [],
      ...
      migrateCanonicalLocalServerState(
        {
          projects: {
            ...
            "https://grexcode.example.com": [
              { worktree: "/local", expanded: true },
              { worktree: "/remote", expanded: true },
            ],
          },
          lastProject: { local: "/local", "https://grexcode.example.com": "/remote" },
        },
        ServerConnection.Key.make("https://grexcode.example.com"),
      ),
    ).toEqual({
