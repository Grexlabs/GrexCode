...
export const GREXCODE_BASE_MODE = "base"
...
export function createGrexcodeModeStack(keymap: OpenTuiKeymap) {
  ...
  const offFields = keymap.registerLayerFields({
    mode(value, ctx) {
      ctx.require(GREXCODE_MODE_KEY, value)
    },
  })
  ...
  const update = () => {
    keymap.setData(GREXCODE_MODE_KEY, stack.at(-1)?.mode ?? GREXCODE_BASE_MODE)
  }

  const stackApi = {
    current() {
      return stack.at(-1)?.mode ?? GREXCODE_BASE_MODE
    },
    push(mode: string) {
      if (disposed) return () => {}
    ...
    dispose() {
      ...
      stack.length = 0
      offFields()
      keymap.setData(GREXCODE_MODE_KEY, undefined)
