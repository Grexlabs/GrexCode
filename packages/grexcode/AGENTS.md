...
- Running `bun dev` from `packages/grexcode` starts the live interactive TUI. Do not run it as a blocking foreground command when you need to inspect the result.
- Start it in `tmux` instead: `tmux new-session -d -s grexcode-dev 'bun dev'`.
- Capture the current TUI output with: `tmux capture-pane -pt grexcode-dev`.
- Stop the session explicitly when done: `tmux kill-session -t grexcode-dev`.
...
export class Service extends Context.Service<Service, Interface>()("@grexcode/Foo") {}
