...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "grexcode.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "grexcode.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "grexcode.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "grexcode.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["grexcode.json", "grexcode.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })
      ...
      expect(result).toEqual([
        ...
        path.join(project, "grexcode.json"),
        path.join(project, "grexcode.jsonc"),
      ])
    })
  })
