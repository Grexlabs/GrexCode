...
import { SessionV1 } from "@grexlabs/core/v1/session"
import { FSUtil } from "@grexlabs/core/fs-util"
import { ModelsDev } from "@grexlabs/core/models-dev"
import { HttpRecorder } from "@grexlabs/http-recorder"
import { HttpRecorderInternal } from "@grexlabs/http-recorder/internal"
...
import { LLMEvent, LLMResponse } from "@grexlabs/llm"
import { LLMClient, RequestExecutor, WebSocketExecutor } from "@grexlabs/llm/route"
import { Env } from "@/env"
...
const zenURL = (connection: string) => `https://console.grexlabs.in/proxy/connections/${connection}/v1`
...
function decodeRecordOpenAIOAuth() {
  const value = process.env.GREXCODE_RECORD_OPENAI_AUTH
...
const RECORDED_SCENARIOS = [
  {
    ...
    tags: ["grexcode", "native", "tool-loop"],
    canRecord: () => Boolean(envValue("GREXCODE_RECORD_OPENAI_API_KEY", "OPENAI_API_KEY")),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.openai,
        name: "OpenAI",
        ...
        model,
        options: {
          apiKey: envValue("GREXCODE_RECORD_OPENAI_API_KEY", "OPENAI_API_KEY") ?? "fixture-openai-key",
  ...
  },
  {
    ...
    protocol: "openai-responses",
    tags: ["grexcode", "native", "oauth", "tool-loop"],
  ...
  },
  {
    id: "grexcode-proxy",
    name: "GrexCode proxy",
    providerID: ProviderV2.ID.grexcode,
    modelID: "gpt-5.2-codex",
    cassette: "session/native-zen-tool-loop",
    protocol: "openai-responses",
    tags: ["grexcode", "zen", "native", "tool-loop"],
    canRecord: () => Boolean(process.env.GREXCODE_RECORD_CONSOLE_TOKEN && process.env.GREXCODE_RECORD_ZEN_ORG_ID),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.grexcode,
        name: "GrexCode Zen",
        env: ["GREXCODE_CONSOLE_TOKEN"],
        npm: "@ai-sdk/openai-compatible",
        api: zenURL(process.env.GREXCODE_RECORD_ZEN_CONNECTION ?? "fixture"),
        model,
        options: {
          apiKey: process.env.GREXCODE_RECORD_CONSOLE_TOKEN ?? "fixture-console-token",
          headers: { "x-org-id": process.env.GREXCODE_RECORD_ZEN_ORG_ID ?? "fixture-org" },
        },
      }),
  },
  {
    ...
    tags: ["grexcode", "native", "tool-loop"],
    canRecord: () => Boolean(envValue("GREXCODE_RECORD_ANTHROPIC_API_KEY", "ANTHROPIC_API_KEY")),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.anthropic,
        name: "Anthropic",
        ...
        model,
        options: {
          apiKey: envValue("GREXCODE_RECORD_ANTHROPIC_API_KEY", "ANTHROPIC_API_KEY") ?? "fixture-anthropic-key",
...
const selectedScenarios = new Set(
  (envValue("GREXCODE_RECORDED_SCENARIO", "RECORDED_PROVIDER") ?? "")
    .split(",")
...
const recordError = (scenario: RecordedScenario) =>
  scenario.id === "openai-oauth"
    ? "Set GREXCODE_RECORD_OPENAI_AUTH to an OAuth auth JSON object in the recording environment."
...
function recordedNativeLLMLayer(scenario: RecordedScenario) {
  ...
  // Only the HTTP client is recorded; RequestExecutor and the grexcode LLM stack remain real.
  const metadata = {
...
const writeConfig = (directory: string, scenario: RecordedScenario, model: ModelsDev.Provider["models"][string]) =>
  Effect.promise(() =>
    Bun.write(
      path.join(directory, "grexcode.json"),
      JSON.stringify({ $schema: "https://grexlabs.in/config.json", ...scenario.config(model) }),
    ),
  )
