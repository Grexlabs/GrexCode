...
import { LocationServiceMap } from "@grexlabs/core/location-layer"
...
it.instance("plan agent denies edits except .grexcode/plans/*", () =>
  Effect.gen(function* () {
    ...
    expect(Permission.evaluate("edit", ".grexcode/plans/foo.md", plan!.permission).action).toBe("allow")
  }),
)
...
it.instance(
  ...
  () =>
    Effect.gen(function* () {
      ...
      const skillDir = path.join(test.directory, ".grexcode", "skill", "perm-skill")
...
`,
      ...
      const home = process.env.GREXCODE_TEST_HOME
      process.env.GREXCODE_TEST_HOME = test.directory
