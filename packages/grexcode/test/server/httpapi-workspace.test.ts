...
function requestServer(path: string, directory: string, init: RequestInit = {}) {
  ...
  headers.set("x-grexcode-directory", directory)
      ...
      try {
        const response = yield* requestDefault(url.toString(), dir, {
          ...
          headers: {
            "accept-encoding": "br",
            "content-type": "application/json",
            "x-grexcode-workspace": "internal",
          },
          body: JSON.stringify({ $schema: "https://grexlabs.in/config.json" }),
        })
        ...
        expect(forwarded[0]?.headers).not.toHaveProperty("x-grexcode-directory")
        expect(forwarded[0]?.headers).not.toHaveProperty("x-grexcode-workspace")
