// Subprocess test harness for the grexcode CLI. Spawns the real binary against
...
//
// Configuration flows through grexcode's built-in test affordances:
//   - GREXCODE_CONFIG_CONTENT      : provider config inline, no files to find
//   - GREXCODE_TEST_HOME           : pins os.homedir() → tmpdir
//   - GREXCODE_DISABLE_PROJECT_CONFIG : skip walking up for grexcode.json
//   - GREXCODE_PURE                : skip external plugin discovery + install
//   - GREXCODE_DISABLE_AUTOUPDATE / AUTOCOMPACT / MODELS_FETCH : no background work
// Plus HOME / XDG_* pointing at the tmpdir for belt-and-suspenders isolation.
//
// Today only `grexcode.run` is fully wired. The shape supports adding more
// builders (`grexcode.serve(opts)`, `grexcode.acp(opts)`, `grexcode.auth(...)`)
// without changing the fixture. Long-lived commands like `serve` will need a
// different return shape — see the TODO at the bottom of OpencodeCli.
import { test, type TestOptions } from "bun:test"
import { FSUtil } from "@grexlabs/core/fs-util"
import { AppProcess } from "@grexlabs/core/process"
...
const grexcodeRoot = path.resolve(import.meta.dir, "../../")
const cliEntry = path.join(grexcodeRoot, "src/index.ts")
...
function isolatedEnv(home: string, configJson: string): Record<string, string> {
  return {
    GREXCODE_TEST_HOME: home,
    HOME: home,
    XDG_CONFIG_HOME: path.join(home, ".config"),
    XDG_DATA_HOME: path.join(home, ".local/share"),
    XDG_STATE_HOME: path.join(home, ".local/state"),
    XDG_CACHE_HOME: path.join(home, ".cache"),
    GREXCODE_CONFIG_CONTENT: configJson,
    GREXCODE_DISABLE_PROJECT_CONFIG: "1",
    GREXCODE_PURE: "1",
    GREXCODE_DISABLE_AUTOUPDATE: "1",
    GREXCODE_DISABLE_AUTOCOMPACT: "1",
    GREXCODE_DISABLE_MODELS_FETCH: "1",
    GREXCODE_AUTH_CONTENT: "{}",
  }
}
...
// Typed equivalent of constructing argv for `grexcode run`. New flags should
...
// `grexcode serve` is a long-lived process — it never exits on its own.
...
// `grexcode acp` speaks newline-delimited JSON-RPC over stdin/stdout. It is
...
export type OpencodeCli = {
  ...
  // Spawn `grexcode serve` and wait until it's listening. Long-lived: the
  ...
  // Spawn `grexcode acp` and return a duplex JSON-RPC handle. Long-lived:
...
export function withCliFixture<A, E>(
  ...
  return Effect.gen(function* () {
    ...
    const spawn = Effect.fn("grexcode.spawn")(function* (args: string[], opts?: SpawnOpts) {
    ...
    const runOpts = (opts?: RunOpts): SpawnOpts | undefined => {
      ...
      return {
        ...
        env: {
          ...opts.env,
          GREXCODE_CONFIG_CONTENT: JSON.stringify({
    ...
    const startRun = Effect.fn("grexcode.startRun")(function* (message: string, opts?: RunOpts) {
      const start = Date.now()
    ...
    const serve = Effect.fn("grexcode.serve")(function* (opts?: ServeOpts) {
      const argv = ["serve"]
      ...
      //   "grexcode server listening on http://<host>:<port>"
      ...
      const match = yield* Deferred.await(readyDeferred).pipe(
        Effect.timeoutOrElse({
          ...
          orElse: () =>
            Effect.fail(
              new Error(
                `grexcode serve did not become ready within ${readyTimeoutMs}ms\n` +
    ...
    const acp = Effect.fn("grexcode.acp")(function* (opts?: AcpOpts) {
      const argv = ["acp"]
    ...
    const grexcode: OpencodeCli = { run, startRun, serve, acp, spawn, expectExit, parseJsonEvents }

    return yield* fn({ llm, home, grexcode })
    // FetchHttpClient is provided so test bodies can `yield* HttpClient.HttpClient`
    // and hit endpoints on `grexcode.serve()` without rolling their own fetch.
  }).pipe(
    Effect.provide(
...
function expectExit(result: RunResult, expected: number, label = "grexcode") {
...
// every call site. Use this for any test that needs the grexcode CLI fixture.
//
...
// resources (e.g. `grexcode.serve`) without an extra `Effect.scoped` wrapper —
