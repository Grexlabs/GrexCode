...
import type { HostPluginApi } from "@grexlabs/tui/plugin/slots"
...
export function createTuiPluginApi(opts: Opts = {}): HostPluginApi {
  ...
  const own = createGrexcodeClient({
  ...
  let selected = opts.theme?.selected ?? "grexcode"
  const set =
    opts.theme?.set ??
    ((name: string) => {
  ...
  return {
    ...
    attention: {
      ...
      soundboard: {
        ...
        current: () => opts.attention?.soundboard?.current?.() ?? "grexcode.default",
