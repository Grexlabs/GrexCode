...
// terminal sizes. The default grexcode tui command is excluded —
// `grexcode --help` includes an ASCII banner that pulls in the install
...
// Top-level commands. Order matches what `grexcode --help` prints today;
...
// top-level help on `--help` and exits 1; not a real grexcode command.
const TOP_LEVEL = [
  "acp",
  "mcp",
  "attach",
...
describe("grexcode CLI help-text snapshots", () => {
  ...
  cliIt.live(
    ...
    ({ grexcode }) =>
      Effect.gen(function* () {
        const topLevel = yield* grexcode.spawn(["--help"], { env: SNAPSHOT_ENV })
        ...
        const [failures, results] = yield* Effect.partition(
          ...
          (argv) =>
            Effect.gen(function* () {
              const result = yield* grexcode.spawn([...argv, "--help"], { env: SNAPSHOT_ENV })
              if (result.exitCode !== 0) {
                return yield* Effect.fail(`grexcode ${argv.join(" ")}: exit ${result.exitCode}`)
              }
              return { argv, result }
            }),
          { concurrency: 8 },
        ...
        for (const { argv, result } of results) {
          ...
          expect(normalize(result.stderr)).toMatchSnapshot(`grexcode ${argv.join(" ")} --help`)
        }
