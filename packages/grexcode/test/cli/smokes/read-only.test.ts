...
describe("grexcode read-only commands (smoke)", () => {
  // `mcp list` reads MCP server config and pings each one. With the empty
  // GREXCODE_CONFIG_CONTENT={} we provide, no servers should be configured
  // and the command should report that cleanly.
  cliIt.live(
    "mcp list: exits 0",
    ({ grexcode }) =>
      Effect.gen(function* () {
        const r = yield* grexcode.spawn(["mcp", "list"])
        grexcode.expectExit(r, 0, "mcp list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ grexcode }) =>
      Effect.gen(function* () {
        const r = yield* grexcode.spawn(["providers", "list"])
        grexcode.expectExit(r, 0, "providers list")
  ...
  cliIt.live(
    ...
    ({ grexcode }) =>
      Effect.gen(function* () {
        const r = yield* grexcode.spawn(["models"])
        grexcode.expectExit(r, 0, "models")
  ...
  cliIt.live(
    ...
    ({ grexcode }) =>
      Effect.gen(function* () {
        const r = yield* grexcode.spawn(["agent", "list"])
        grexcode.expectExit(r, 0, "agent list")
      }),
    60_000,
  )

  // `session list` reads the session DB. Fresh GREXCODE_TEST_HOME means
  // empty DB. Exit 0 with no sessions.
  cliIt.live(
    "session list: exits 0",
    ({ grexcode }) =>
      Effect.gen(function* () {
        const r = yield* grexcode.spawn(["session", "list"])
        grexcode.expectExit(r, 0, "session list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ grexcode }) =>
      Effect.gen(function* () {
        const r = yield* grexcode.spawn(["stats"])
        grexcode.expectExit(r, 0, "stats")
      }),
    60_000,
  )
  ...
  // in production it'd be a path under GREXCODE_TEST_HOME / XDG_DATA_HOME.
  // Accept either form — both prove the resolver ran without crashing.
  cliIt.live(
    "db path: exits 0 and prints a path or :memory:",
    ({ grexcode }) =>
      Effect.gen(function* () {
        const r = yield* grexcode.spawn(["db", "path"])
        grexcode.expectExit(r, 0, "db path")
