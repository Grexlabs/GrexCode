...
describe("tui thread", () => {
  ...
  cliIt.live("rejects mini-only options without --mini", ({ grexcode }) =>
    Effect.gen(function* () {
      const result = yield* grexcode.spawn(["--replay-limit", "10"])

      grexcode.expectExit(result, 1)
      expect(result.stderr).toContain("--replay-limit requires --mini")
    }),
  )

  cliIt.live("routes attached sessions to mini mode", ({ grexcode }) =>
    Effect.gen(function* () {
      const result = yield* grexcode.spawn(["attach", "http://127.0.0.1:1", "--mini"])
  ...
  cliIt.live("rejects network options in mini mode", ({ grexcode }) =>
    Effect.gen(function* () {
      const result = yield* grexcode.spawn(["--mini", "--port", "4096"])
