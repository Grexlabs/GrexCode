// Subprocess integration tests for `grexcode run` (non-interactive mode).
// These exercise the real CLI binary against a TestLLMServer running in the
// same process. See `test/lib/cli-process.ts` for the harness — each test uses
// `grexcode.run(message, opts?)` to spawn `bun src/index.ts run ...` with
// `GREXCODE_CONFIG_CONTENT` providing the test provider config inline.
import { describe, expect } from "bun:test"
import { Effect } from "effect"
import { reply } from "../../lib/llm-server"
import { cliIt } from "../../lib/cli-process"

describe("grexcode run (non-interactive subprocess)", () => {
  ...
  cliIt.concurrent(
    ...
    ({ llm, grexcode }) =>
      Effect.gen(function* () {
        ...
        const result = yield* grexcode.run("say hi")
        grexcode.expectExit(result, 0)
  ...
  cliIt.concurrent(
    ...
    ({ llm, grexcode }) =>
      Effect.gen(function* () {
        ...
        const result = yield* grexcode.run("use a tool", {
  ...
  cliIt.concurrent(
    ...
    ({ llm, grexcode }) =>
      Effect.gen(function* () {
        ...
        const thinking = yield* grexcode.run("think", { extraArgs: ["--thinking"] })
        grexcode.expectExit(thinking, 0)
        expect(thinking.stdout).toBe("Thinking: considering\nanswer\n")

        yield* llm.reason("hidden", { text: "visible" })
        const plain = yield* grexcode.run("think again")
        grexcode.expectExit(plain, 0)
  ...
  cliIt.concurrent(
    ...
    ({ llm, grexcode }) =>
      Effect.gen(function* () {
        ...
        const result = yield* grexcode.run("trigger midstream error", { timeoutMs: 30_000 })
  ...
  cliIt.concurrent(
    ...
    ({ llm, grexcode }) =>
      Effect.gen(function* () {
        ...
        const result = yield* grexcode.run("say hi", { format: "json" })
        grexcode.expectExit(result, 0)

        const events = grexcode.parseJsonEvents(result.stdout)
  ...
  cliIt.concurrent(
    ...
    ({ grexcode }) =>
      Effect.gen(function* () {
        const result = yield* grexcode.run("use an unknown model", {
          model: "test/nonexistent-model",
          format: "json",
        })
  ...
  cliIt.concurrent(
    ...
    ({ llm, grexcode }) =>
      Effect.gen(function* () {
        ...
        const result = yield* grexcode.run("exercise json records", {
          format: "json",
  ...
  cliIt.concurrent(
    ...
    ({ llm, grexcode }) =>
      Effect.gen(function* () {
        ...
        const result = yield* grexcode.run("fail after output", { format: "json" })
  ...
  cliIt.concurrent(
    ...
    ({ home, llm, grexcode }) =>
      Effect.gen(function* () {
        ...
        const denied = yield* grexcode.run("request permission", { permission: { bash: "ask" } })
        grexcode.expectExit(denied, 0)
        ...
        const allowed = yield* grexcode.run("request permission", {
          permission: { bash: "ask" },
          extraArgs: ["--dangerously-skip-permissions"],
        })
        grexcode.expectExit(allowed, 0)
        ...
        const explicitlyDenied = yield* grexcode.run("request denied permission", {
          permission: { bash: "deny" },
          extraArgs: ["--dangerously-skip-permissions"],
        })
        grexcode.expectExit(explicitlyDenied, 0)
  ...
  cliIt.live(
    ...
    ({ home, llm, grexcode }) =>
      Effect.gen(function* () {
        ...
        const server = yield* grexcode.serve()

        const result = yield* grexcode.run("read the attachment", {
  ...
  cliIt.concurrent(
    ...
    ({ home, grexcode }) =>
      Effect.gen(function* () {
        const result = yield* grexcode.run("read the directory", {
  ...
  cliIt.live(
    ...
    ({ llm, grexcode }) =>
      Effect.gen(function* () {
        yield* llm.hang
        const run = yield* grexcode.startRun("wait forever")
        yield* llm.wait(1)
        run.interrupt()
