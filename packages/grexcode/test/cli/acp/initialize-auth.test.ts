...
describe("grexcode acp initialize/auth subprocess", () => {
  cliIt.live(
    "initialize responds with capabilities",
    ({ grexcode }) =>
      Effect.gen(function* () {
        const initialized = yield* initialize(yield* createAcpClient({ grexcode }))
        ...
        expect(initialized.agentInfo?.name).toBe("GrexCode")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ grexcode }) =>
      Effect.gen(function* () {
        const acp = yield* createAcpClient({ grexcode })
        const initialized = yield* initialize(acp)

        expect(initialized.authMethods?.[0]?.id).toBe("grexcode-login")
        expect(initialized.authMethods?.[0]?._meta?.["terminal-auth"]).toBeDefined()
        expect(yield* acp.request<AuthenticateResponse>("authenticate", { methodId: "grexcode-login" })).toMatchObject({
          result: {},
        })
        ...
        expect(JSON.stringify(rejected.error)).not.toContain(process.env.GREXCODE_AUTH_CONTENT ?? "not-present")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ grexcode }) =>
      Effect.gen(function* () {
        ...
        expect(initialized.result?.authMethods?.[0]?.id).toBe("grexcode-login")
