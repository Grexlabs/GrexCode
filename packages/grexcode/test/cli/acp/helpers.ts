...
export function createAcpClient(input: Pick<CliFixture, "grexcode">, env?: Record<string, string>) {
  return Effect.gen(function* () {
    return createJsonRpcAcpClient(yield* input.grexcode.acp(env ? { env } : undefined))
  })
}
  ...
  return Effect.gen(function* () {
    return expectOk(
      yield* acp.request<InitializeResponse>("initialize", {
        ...
        clientInfo: { name: "grexcode-local-acp", version: "0.1.0" },
      }),
    )
  })
}
