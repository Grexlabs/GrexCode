...
import { EventSequenceTable } from "@grexlabs/core/event/sql"
...
import { WorkspaceV2 } from "@grexlabs/core/workspace"
import { WorkspaceTable } from "@grexlabs/core/control-plane/workspace.sql"
...
const originalEnv = {
  GREXCODE_AUTH_CONTENT: process.env.GREXCODE_AUTH_CONTENT,
  GREXCODE_EXPERIMENTAL_WORKSPACES: process.env.GREXCODE_EXPERIMENTAL_WORKSPACES,
...
beforeEach(() => {
  restoreEnv()
  process.env.GREXCODE_EXPERIMENTAL_WORKSPACES = "true"
})
...
async function initGitRepo(dir: string) {
  ...
  await $`git config user.email "test@grexcode.test"`.cwd(dir).quiet()
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        const instance = yield* requireInstance
        const workspace = yield* Workspace.Service
        process.env.GREXCODE_AUTH_CONTENT = JSON.stringify({ test: { type: "api", key: "secret" } })
        process.env.OTEL_EXPORTER_OTLP_HEADERS = "authorization=otel"
        process.env.OTEL_EXPORTER_OTLP_ENDPOINT = "https://otel.test"
        process.env.OTEL_RESOURCE_ATTRIBUTES = "service.name=grexcode-test"
        ...
        expect(JSON.parse(recorded.calls.create[0].env.GREXCODE_AUTH_CONTENT ?? "{}")).toEqual({
          test: { type: "api", key: "secret" },
        })
        expect(recorded.calls.create[0].env.GREXCODE_WORKSPACE_ID).toBe(workspaceID)
        expect(recorded.calls.create[0].env.GREXCODE_EXPERIMENTAL_WORKSPACES).toBe("true")
        ...
        expect(recorded.calls.create[0].env.OTEL_RESOURCE_ATTRIBUTES).toBe("service.name=grexcode-test")
