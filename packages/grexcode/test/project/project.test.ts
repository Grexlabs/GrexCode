...
import { ProjectDirectories } from "@grexlabs/core/project/directories"
...
describe("Project.fromDirectory", () => {
  it.live("should handle git repository with no commits", () =>
    Effect.gen(function* () {
      ...
      const grexcodeFile = path.join(tmp, ".git", "grexcode")
      expect(yield* Effect.promise(() => Bun.file(grexcodeFile).exists())).toBe(false)
    }),
  )
...
describe("Project.fromDirectory with worktrees", () => {
  ...
  it.live("worktree should share project ID with main repo", () =>
    Effect.gen(function* () {
      ...
      const cache = path.join(tmp, ".git", "grexcode")
...
describe("Project.fromDirectory with bare repos", () => {
  it.live("worktree from bare repo should cache in bare repo, not parent", () =>
    Effect.gen(function* () {
      ...
      const correctCache = path.join(barePath, "grexcode")
      const wrongCache = path.join(parentDir, ".git", "grexcode")
  ...
  it.live("different bare repos under same parent should not share project ID", () =>
    Effect.gen(function* () {
      ...
      const cacheA = path.join(bareA, "grexcode")
      const cacheB = path.join(bareB, "grexcode")
      const wrongCache = path.join(parentDir, ".git", "grexcode")
