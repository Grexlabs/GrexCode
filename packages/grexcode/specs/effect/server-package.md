...
Practical reference for a future `packages/server` split after the grexcode
server moved to the Effect HttpApi backend.

## Current State

- The server still lives in `packages/grexcode`.
...
Until enough shared service code lives outside `packages/grexcode`, a future
`packages/server` should either:

- own pure HttpApi contracts only, or
- accept host-provided services/layers/callbacks from `packages/grexcode`

It should not import `packages/grexcode` services while `packages/grexcode`
...
2. Move stable domain schemas into shared packages only when they no longer
   depend on grexcode-local runtime modules.
3. Extract pure HttpApi contract modules into `packages/server` once the contract
   can compile without importing `packages/grexcode` implementation details.
