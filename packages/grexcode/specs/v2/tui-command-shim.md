...
Delete `packages/grexcode/src/cli/cmd/tui/plugin/command-shim.ts`.

In `packages/grexcode/src/cli/cmd/tui/plugin/api.tsx`, remove:

- the `createCommandShim` import
- the `command: createCommandShim(...)` field from `createTuiApi(...)`

In `packages/grexcode/src/cli/cmd/tui/plugin/runtime.ts`, remove:
...
- `bun typecheck` in `packages/grexcode`
- TUI plugin loader tests in `packages/grexcode` if runtime plugin API wiring changed
