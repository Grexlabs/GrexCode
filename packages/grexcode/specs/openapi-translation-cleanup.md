...
Trim `packages/grexcode/src/server/routes/instance/httpapi/public.ts` until OpenAPI generation is mostly a direct projection of the `HttpApi` route declarations, without breaking the generated SDK surface.
...
- `[x]` Add or extend `packages/grexcode/test/server/httpapi-query-schema-drift.test.ts`.
...
- `[x]` `bun test --timeout 5000 test/server/httpapi-query-schema-drift.test.ts` from `packages/grexcode`.
- `[x]` `bun typecheck` from `packages/grexcode`.

### PR 2: Delete Spec-Only Workspace Query Injection

- `[x]` Edit `packages/grexcode/src/server/routes/instance/httpapi/public.ts`.
...
- `[x]` `bun test --timeout 5000 test/server/httpapi-query-schema-drift.test.ts` from `packages/grexcode`.
- `[x]` `bun dev generate > /tmp/grexcode-openapi.json` from `packages/grexcode`.
...
- Edit `packages/grexcode/src/server/routes/instance/httpapi/public.ts`.
...
- `bun dev generate > /tmp/grexcode-openapi.json` from `packages/grexcode`.
...
- Check source schemas in files like `packages/grexcode/src/session/schema.ts`, `packages/grexcode/src/permission/schema.ts`, and pty schema definitions.
...
- Edit route group files under `packages/grexcode/src/server/routes/instance/httpapi/groups/`.
- Replace SDK-visible `HttpApiError.BadRequest` / `HttpApiError.NotFound` with explicit error schemas from `packages/grexcode/src/server/routes/instance/httpapi/errors.ts` or add new ones there.
