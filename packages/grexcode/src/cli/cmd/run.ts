import type { PermissionV1 } from "@grexlabs/core/v1/permission"
import { FSUtil } from "@grexlabs/core/fs-util"
// CLI entry point for `grexcode run` and `grexcode --mini`.
//
// Handles three modes:
//   1. Non-interactive (default): sends a single prompt, streams events to
//      stdout, and exits when the session goes idle.
//   2. Interactive local (`grexcode --mini`): boots the split-footer direct mode
//      with an in-process server (no external HTTP).
//   3. Interactive attach (`grexcode --mini --attach`): connects to a running
//      grexcode server and runs interactive mode against it.
//
...
import { createGrexcodeClient, type GrexcodeClient, type ToolPart } from "@grexlabs/sdk/v2"
import { FormatError, FormatUnknownError } from "../error"
import { INTERACTIVE_INPUT_ERROR, resolveInteractiveStdin } from "./run/runtime.stdin"

type ModelInput = Parameters<GrexcodeClient["session"]["prompt"]>[0]["model"]
...
export const RunCommand = effectCmd({
  command: "run [message..]",
  describe: "run grexcode with a message",
  ...
  builder: (yargs: Argv) =>
    yargs
      ...
      .option("attach", {
        type: "string",
        describe: "attach to a running grexcode server (e.g., http://localhost:4096)",
      })
      .option("password", {
        alias: ["p"],
        type: "string",
        describe: "basic auth password (defaults to GREXCODE_SERVER_PASSWORD)",
      })
      .option("username", {
        alias: ["u"],
        type: "string",
        describe: "basic auth username (defaults to GREXCODE_SERVER_USERNAME or 'grexcode')",
      })
      .option("dir", {
        type: "string",
  ...
  handler: Effect.fn("Cli.run")(function* (args) {
    ...
    yield* Effect.promise(async () => {
      ...
      async function session(sdk: GrexcodeClient): Promise<SessionInfo | undefined> {
        if (args.session) {
      ...
      async function share(sdk: GrexcodeClient, sessionID: string) {
      ...
      async function current(sdk: GrexcodeClient): Promise<string> {
        if (!args.attach) {
      ...
      async function attachAgent(sdk: GrexcodeClient) {
        if (!args.agent) return undefined
        const name = args.agent
      ...
      async function pickAgent(sdk: GrexcodeClient) {
        if (!args.agent) return undefined
        if (args.attach) {
      ...
      async function execute(sdk: GrexcodeClient) {
        ...
        async function loop(client: GrexcodeClient, events: Awaited<ReturnType<typeof sdk.event.subscribe>>) {
      ...
      const sdk = createGrexcodeClient({
        baseUrl: "http://grexcode.internal",
        fetch: fetchFn,
        directory,
      })
      await execute(sdk)
