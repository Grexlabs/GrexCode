// Shared type vocabulary for the direct interactive mode (`grexcode --mini`).
//
...
import type { GrexcodeClient, PermissionRequest, QuestionRequest, ToolPart } from "@grexlabs/sdk/v2"
import type { TuiConfig } from "@grexlabs/tui/config"
...
type PromptModel = Parameters<GrexcodeClient["session"]["prompt"]>[0]["model"]
type PromptInput = Parameters<GrexcodeClient["session"]["prompt"]>[0]

export type RunPromptPart = NonNullable<PromptInput["parts"]>[number]

export type RunCommand = NonNullable<Awaited<ReturnType<GrexcodeClient["command"]["list"]>>["data"]>[number]

export type RunProvider = NonNullable<Awaited<ReturnType<GrexcodeClient["provider"]["list"]>>["data"]>["all"][number]
...
export type RunAgent = NonNullable<Awaited<ReturnType<GrexcodeClient["app"]["agents"]>>["data"]>[number]

type RunResourceMap = NonNullable<Awaited<ReturnType<GrexcodeClient["experimental"]["resource"]["list"]>>["data"]>
...
export type PermissionReply = Parameters<GrexcodeClient["permission"]["reply"]>[0]

export type QuestionReply = Parameters<GrexcodeClient["question"]["reply"]>[0]

export type QuestionReject = Parameters<GrexcodeClient["question"]["reject"]>[0]
