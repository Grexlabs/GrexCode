...
import {
  ...
  type TuiSlotPlugin,
  type TuiTheme,
} from "@grexlabs/plugin/tui"
import path from "path"
import { fileURLToPath } from "url"
import { TuiConfig } from "@/config/tui"
import { errorData, errorMessage } from "@grexlabs/tui/util/error"
...
import { hasTheme, upsertTheme } from "@grexlabs/tui/context/theme"
import { Global } from "@grexlabs/core/global"
import { Filesystem } from "@/util/filesystem"
import { Process } from "@/util/process"
import { Flock } from "@grexlabs/core/util/flock"
import { Flag } from "@grexlabs/core/flag/flag"
import { internalTuiPlugins, type InternalTuiPlugin } from "./internal"
import type { HostPluginApi, HostSlots } from "@grexlabs/tui/plugin/slots"
import { ConfigPlugin } from "@/config/plugin"
import { ConfigPluginV1 } from "@grexlabs/core/v1/config/plugin"
import { createCommandShim } from "@grexlabs/tui/plugin/command-shim"
import { RuntimeFlags } from "@/effect/runtime-flags"
import { Effect } from "effect"
import { createPluginRuntime, type PluginRuntime, type TuiPluginHost } from "@grexlabs/tui/plugin/runtime"
...
function createThemeInstaller(
  ...
  return async (file) => {
    ...
    const local_dir =
      path.basename(source_dir) === ".grexcode"
        ? path.join(source_dir, "themes")
        : path.join(source_dir, ".grexcode", "themes")
...
function defaultPluginOrigin(state: RuntimeState, spec: string): ConfigPlugin.Origin {
  return {
    spec,
    scope: "local",
    source: state.api.state.path.config || path.join(state.directory, ".grexcode", "tui.json"),
  }
}
...
async function load(input: {
  ...
  try {
    ...
    const records = Flag.GREXCODE_PURE ? [] : pluginOrigins
    if (Flag.GREXCODE_PURE && pluginOrigins.length) {
    }
