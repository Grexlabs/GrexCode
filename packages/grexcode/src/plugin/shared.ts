...
export const DEPRECATED_PLUGIN_PACKAGES = ["grexcode-openai-codex-auth", "grexcode-copilot-auth"]
...
export async function checkPluginCompatibility(target: string, grexcodeVersion: string, pkg?: PluginPackage) {
  if (!semver.valid(grexcodeVersion) || semver.major(grexcodeVersion) === 0) return
  const hit = pkg ?? (await readPluginPackage(target).catch(() => undefined))
  if (!hit) return
  const engines = hit.json.engines
  if (!isRecord(engines)) return
  const range = engines.grexcode
  if (typeof range !== "string") return
  if (!semver.satisfies(grexcodeVersion, range)) {
    throw new Error(`Plugin requires grexcode ${range} but running ${grexcodeVersion}`)
  }
}
