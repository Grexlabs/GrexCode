...
import type { Message, GrexcodeClient, SessionMessageResponse } from "@grexlabs/sdk/v2"
...
export const AuthMethodID = "grexcode-login"
...
export class Service extends Context.Service<Service, Interface>()("@grexcode/ACP/Service") {}
...
export function make(input: {
  ...
  const initialize = Effect.fn("ACP.initialize")(function* (params: InitializeRequest) {
    ...
    const authMethod: AuthMethod = {
      description: "Run `grexcode auth login` in the terminal",
      name: "Login with grexcode",
      id: AuthMethodID,
    }
...
function makeDirectoryService(sdk: GrexcodeClient) {
...
function makeUsageService(sdk: GrexcodeClient) {
...
async function loadDirectorySnapshot(sdk: GrexcodeClient, directory: string) {
...
function defaultModelFromConfig(
  ...
  // a default. Configured model, grexcode provider, then sorted best model keep
  // the protocol response deterministic without extra session/message reads.
  const grexcodeProvider = providers[ProviderV2.ID.make("grexcode")]
  const grexcodeModel = grexcodeProvider ? Provider.sort(Object.values(grexcodeProvider.models))[0] : undefined
  if (grexcodeProvider && grexcodeModel) return { providerID: grexcodeProvider.id, modelID: grexcodeModel.id }
...
function fromUnknownError(error: unknown, service?: string): Error {
  ...
  return new ACPError.ServiceFailureError({ safeMessage: "GrexCode service failure", service })
}
