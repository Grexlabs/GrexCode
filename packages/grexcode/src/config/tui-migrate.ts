...
const TUI_SCHEMA_URL = "https://grexlabs.in/tui.json"
...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from grexcode.json files
 * into dedicated tui.json files. Migration is performed per-directory and
 * skips only locations where a tui.json already exists.
 */
export async function migrateTuiConfig(input: MigrateInput) {
  const grexcode = await grexcodeFiles(input)
  for (const file of grexcode) {
...
async function grexcodeFiles(input: { directories: string[]; cwd: string }) {
  const files = [
    ...ConfigPaths.fileInDirectory(Global.Path.config, "grexcode"),
    ...(await Filesystem.findUp(["grexcode.json", "grexcode.jsonc"], input.cwd, undefined, { rootFirst: true })),
  ]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "grexcode"))
  }
  if (Flag.GREXCODE_CONFIG) files.push(Flag.GREXCODE_CONFIG)
