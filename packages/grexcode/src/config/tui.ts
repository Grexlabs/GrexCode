...
import { isRecord } from "@grexlabs/tui/util/record"
...
import { TuiKeybind } from "@grexlabs/tui/config/keybind"
...
import { TuiConfig } from "@grexlabs/tui/config"
...
export class Service extends Context.Service<Service, Interface>()("@grexcode/TuiConfig") {}
...
const loadState = Effect.fn("TuiConfig.loadState")(function* (ctx: { directory: string }) {
  ...
  const load = (text: string, configFilepath: string): Effect.Effect<Info> =>
    Effect.gen(function* () {
      ...
      // (mirroring the old grexcode.json shape) still get their settings applied.
  ...
  // Every config dir we may read from: global config dir, any `.grexcode`
  // folders between cwd and home, and GREXCODE_CONFIG_DIR.
  const directories = yield* ConfigPaths.directories(ctx.directory)
  yield* Effect.promise(() => migrateTuiConfig({ directories, cwd: ctx.directory }))

  const projectFiles = Flag.GREXCODE_DISABLE_PROJECT_CONFIG ? [] : yield* ConfigPaths.files("tui", ctx.directory)
  ...
  // 2. Explicit GREXCODE_TUI_CONFIG override, if set.
  if (Flag.GREXCODE_TUI_CONFIG) {
    const configFile = Flag.GREXCODE_TUI_CONFIG
  ...
  // 4. `.grexcode` directories (and GREXCODE_CONFIG_DIR) discovered while
  // walking up the tree. Also returned below so callers can install plugin
  // dependencies from each location.
  const dirs = unique(directories).filter((dir) => dir.endsWith(".grexcode") || dir === Flag.GREXCODE_CONFIG_DIR)

  for (const dir of dirs) {
    if (!dir.endsWith(".grexcode") && dir !== Flag.GREXCODE_CONFIG_DIR) continue
