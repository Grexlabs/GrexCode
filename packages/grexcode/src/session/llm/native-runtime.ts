...
import type { LLMClientShape } from "@grexlabs/llm/route"
...
function statusWithFetch(
  ...
  if (providerID !== "openai" && providerID !== "anthropic" && !providerID.startsWith("grexcode"))
    return { type: "unsupported", reason: "provider is not openai, grexcode, or anthropic" }
...
export function stream(input: StreamInput): StreamResult {
  ...
  // Integration point with @grexlabs/llm: native-request lowers session data
...
export function nativeTools(tools: Record<string, Tool>, input: Pick<StreamInput, "messages" | "abort">) {
  return Object.fromEntries(
    Object.entries(tools).map(([name, item]) => [
      name,
      // Tool execution remains grexcode-owned. The native runtime only adapts
      // the @grexlabs/llm tool call back into the AI SDK Tool.execute shape.
      NativeTool.make({
