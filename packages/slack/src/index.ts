...
import { createGrexcode, type ToolPart } from "@grexlabs/sdk"
...
console.log("🚀 Starting grexcode server...")
const grexcode = await createGrexcode({
  port: 0,
})
console.log("✅ GrexCode server ready")

const sessions = new Map<string, { client: any; server: any; sessionId: string; channel: string; thread: string }>()
void (async () => {
  const events = await grexcode.client.event.subscribe()
...
app.message(async ({ message, say }) => {
  ...
  if (!session) {
    console.log("🆕 Creating new grexcode session...")
    const { client, server } = grexcode
    ...
    console.log("✅ Created grexcode session:", createResult.data.id)
  ...
  console.log("📝 Sending to grexcode:", message.text)
  const result = await session.client.session.prompt({
    path: { id: session.sessionId },
    body: { parts: [{ type: "text", text: message.text }] },
  })

  console.log("📤 GrexCode response:", JSON.stringify(result, null, 2))
