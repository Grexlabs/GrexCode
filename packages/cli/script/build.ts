...
for (const item of targets) {
  ...
  const result = await Bun.build({
    ...
    define: {
      ...
      GREXCODE_CLI_NAME: `'${binary}'`,
      GREXCODE_MODELS_DEV: modelsData,
      GREXCODE_CHANNEL: `'${Script.channel}'`,
      GREXCODE_LIBC: item.os === "linux" ? `'${item.abi ?? "glibc"}'` : "undefined",
    ...
    JSON.stringify(
      {
        ...
        license: "MIT",
        repository: { type: "git", url: "git+https://github.com/grexlabs/grexcode.git" },
        os: [item.os],
        cpu: [item.arch],
      },
      null,
