...
Schema-first LLM core for grexcode. One typed request, response, event, and tool language; provider quirks live in adapters, not in calling code.

```ts
import { Effect } from "effect"
import { LLM, LLMClient } from "@grexlabs/llm"
import { OpenAI } from "@grexlabs/llm/providers"
...
```ts
import { OpenAI, CloudflareAIGateway } from "@grexlabs/llm/providers"
