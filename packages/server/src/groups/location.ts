...
function ref(request: HttpServerRequest.HttpServerRequest): Location.Ref {
  ...
  const workspaceID = query.get("location[workspace]") || request.headers["x-grexcode-workspace"]
  const directory =
    query.get("location[directory]") ||
    (request.headers["x-grexcode-directory"] ? decode(request.headers["x-grexcode-directory"]) : process.cwd())
