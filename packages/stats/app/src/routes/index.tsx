...
const statsHomeTitle = "AI Model Usage Rankings | GrexCode Data"
const statsHomeDescription =
  "Explore GrexCode Go usage across AI models, including token volume, rankings, market share, token pricing, session cost, cache ratio, and geo breakdowns."
const statsHomeFallbackUrl = "https://grexlabs.in/data/"
...
function getStatsHomeUrl(base: string, requestUrl: string) {
  ...
  if (url.hostname === "stats.grexlabs.in") return "https://grexlabs.in/data/"
  if (url.hostname === "stats.dev.grexlabs.in") return "https://dev.grexlabs.in/data/"
  return url.toString()
}
  ...
  return (
    <section id="top-models" data-section="top-models">
      <h2 data-slot="top-models-title">
        <strong>Top models.</strong> <span>Usage of models across GrexCode Go.</span>
      </h2>
      <Show
...
function UniqueUsersSection(props: { data: StatsHomeData["users"] }) {
  ...
  return (
    <section id="unique-users" data-section="unique-users">
      ...
      <SectionTitle title="Unique Users" description="Daily unique GrexCode Go users by model." />
      <Show
        when={data().some((item) => usageTotal(item) > 0)}
        fallback={
