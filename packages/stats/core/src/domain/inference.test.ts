...
describe("inference stat normalization", () => {
  ...
  test("uses provider.model to resolve grexcode route providers", () => {
    ...
    expect(statProvider("big-pickle", "claude-sonnet-4-5", "grexcode")).toBe("anthropic")
    expect(statProvider("big-pickle", "gpt-5", "grexcode")).toBe("openai")
    expect(statProvider("big-pickle", "", "grexcode")).toBe("unknown")
  ...
  test("model aggregates prefer provider.model and use normalized model", () => {
    ...
    expect(
      toModelAggregate({ ...aggregate("big-pickle", "grexcode"), provider_model: "claude-sonnet-4-5" }),
    ).toMatchObject([
      {
        provider: "anthropic",
  ...
  test("provider aggregates never keep grexcode as the provider", () => {
    expect(toProviderAggregate({ ...aggregate("big-pickle", "grexcode"), provider_model: "gpt-5" })).toMatchObject([
      { provider: "openai" },
    ])
    expect(toProviderAggregate(aggregate("big-pickle", "grexcode"))).toMatchObject([{ provider: "unknown" }])
  })

  test("geo aggregates never keep grexcode or big-pickle dimensions", () => {
    expect(toGeoAggregate({ ...aggregate("big-pickle", "grexcode"), country: "US" })).toMatchObject([
