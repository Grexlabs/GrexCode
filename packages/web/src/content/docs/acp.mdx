...
```lua {6-8}
{
  acp_providers = {
    ["opencode"] = {
      ...
      args = { "acp" },
      env = {
        GREXCODE_API_KEY = os.getenv("GREXCODE_API_KEY")
      }
    }
  }
}
...
If you need to pass environment variables (like `GREXCODE_API_KEY`), refer to [Configuring Adapters: Environment Variables](https://codecompanion.olimorris.dev/getting-started#setting-an-api-key) in the CodeCompanion.nvim documentation for full details.
