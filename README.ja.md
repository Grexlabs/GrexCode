...
# パッケージマネージャー
npm i -g grexcode-ai@latest        # bun/pnpm/yarn でもOK
scoop install grexcode             # Windows
choco install grexcode             # Windows
brew install grexlabs/tap/grexcode # macOS と Linux（推奨。常に最新）
brew install grexcode              # macOS と Linux（公式 brew formula。更新頻度は低め）
sudo pacman -S grexcode            # Arch Linux (Stable)
paru -S grexcode-bin               # Arch Linux (Latest from AUR)
mise use -g grexcode               # どのOSでも
nix run nixpkgs#grexcode           # または github:grexlabs/grexcode で最新 dev ブランチ
```
...
GrexCode はデスクトップアプリとしても利用できます。[releases page](https://github.com/grexlabs/grexcode/releases) から直接ダウンロードするか、[grexlabs.in/download](https://grexlabs.in/download) を利用してください。
...
4. `$HOME/.grexcode/bin` - デフォルトのフォールバック
...
GrexCode には組み込みの Agent が2つあり、`Tab` キーで切り替えられます。
...
[agents](https://grexlabs.in/docs/agents) の詳細はこちら。

### ドキュメント

GrexCode の設定については [**ドキュメント**](https://grexlabs.in/docs) を参照してください。

### コントリビュート

GrexCode に貢献したい場合は、Pull Request を送る前に [contributing docs](./CONTRIBUTING.md) を読んでください。

### GrexCode の上に構築する

GrexCode に関連するプロジェクトで、名前に "grexcode"（例: "grexcode-dashboard" や "grexcode-mobile"）を含める場合は、そのプロジェクトが GrexCode チームによって作られたものではなく、いかなる形でも関係がないことを README に明記してください。

---

**コミュニティに参加** [Discord](https://discord.gg/grexcode) | [X.com](https://x.com/grexcode)
