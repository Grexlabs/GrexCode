...
# Pakkehåndteringer
npm i -g grexcode-ai@latest        # eller bun/pnpm/yarn
scoop install grexcode             # Windows
choco install grexcode             # Windows
brew install grexlabs/tap/grexcode # macOS og Linux (anbefalet, altid up to date)
brew install grexcode              # macOS og Linux (officiel brew formula, opdateres sjældnere)
sudo pacman -S grexcode            # Arch Linux (Stable)
paru -S grexcode-bin               # Arch Linux (Latest from AUR)
mise use -g grexcode               # alle OS
nix run nixpkgs#grexcode           # eller github:grexlabs/grexcode for nyeste dev-branch
```
...
GrexCode findes også som desktop-app. Download direkte fra [releases-siden](https://github.com/grexlabs/grexcode/releases) eller [grexlabs.in/download](https://grexlabs.in/download).
...
GrexCode har to indbyggede agents, som du kan skifte mellem med `Tab`-tasten.
...
Læs mere om [agents](https://grexlabs.in/docs/agents).

### Dokumentation

For mere info om konfiguration af GrexCode, [**se vores docs**](https://grexlabs.in/docs).

### Bidrag

Hvis du vil bidrage til GrexCode, så læs vores [contributing docs](./CONTRIBUTING.md) før du sender en pull request.

### Bygget på GrexCode

Hvis du arbejder på et projekt der er relateret til GrexCode og bruger "grexcode" som en del af navnet; f.eks. "grexcode-dashboard" eller "grexcode-mobile", så tilføj en note i din README, der tydeliggør at projektet ikke er bygget af GrexCode-teamet og ikke er tilknyttet os på nogen måde.

---

**Bliv en del af vores community** [Discord](https://discord.gg/grexcode) | [X.com](https://x.com/grexcode)
