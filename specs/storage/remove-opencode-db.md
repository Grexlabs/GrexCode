# Remove `packages/grexcode/src/storage/db.ts`

## Goal

Remove all production usages of the legacy `packages/grexcode/src/storage/db.ts` module.
...
This does not mean removing SQLite or Drizzle everywhere in one step. The smaller target is deleting the grexcode legacy wrapper by moving call sites onto deeper modules or onto the core/effect database adapter directly.

## Current Inventory

Production imports from `packages/grexcode/src/storage/db.ts` are concentrated in 22 source files:

- `packages/grexcode/src/account/repo.ts`
- `packages/grexcode/src/cli/cmd/db.ts`
- `packages/grexcode/src/cli/cmd/import.ts`
- `packages/grexcode/src/cli/cmd/stats.ts`
- `packages/grexcode/src/control-plane/workspace.ts`
- `packages/grexcode/src/index.ts`
- `packages/grexcode/src/node.ts`
- `packages/grexcode/src/permission/index.ts`
- `packages/grexcode/src/project/project.ts`
- `packages/grexcode/src/server/projectors.ts`
- `packages/grexcode/src/server/routes/instance/httpapi/handlers/sync.ts`
- `packages/grexcode/src/server/shared/fence.ts`
- `packages/grexcode/src/session/message-v2.ts`
- `packages/grexcode/src/session/projectors.ts`
- `packages/grexcode/src/session/prompt.ts`
- `packages/grexcode/src/session/session.ts`
- `packages/grexcode/src/session/todo.ts`
- `packages/grexcode/src/share/share-next.ts`
- `packages/grexcode/src/storage/db.ts`
- `packages/grexcode/src/sync/index.ts`
- `packages/grexcode/src/worktree/index.ts`
...
Status: Completed. Startup, the public node export, and database CLI tooling no longer import the legacy grexcode database wrapper; `packages/grexcode/src/storage/db.ts` has been deleted.
...
- Move database path and client startup behind the core/effect database module rather than the grexcode wrapper.
...
Status: Completed. `SyncEvent` and the grexcode projector boundary were removed; session/message event projection now lives in core EventV2/projector infrastructure.
...
Status: Completed. These services no longer import the legacy grexcode database wrapper.
...
Status: Completed. Session/message reads and projector writes have moved off the legacy grexcode database wrapper.
...
Status: Completed. Remaining one-off CLI/admin reads and writes now use core database services or domain services instead of the legacy grexcode database wrapper.
...
All migration groups are complete or superseded. `packages/grexcode/src/storage/db.ts` has been deleted.

## Superseded: Data Migrations

Status: Superseded. No grexcode data-migration group remains.

The previous grexcode `data-migration.ts` service only backfilled session usage from message rows. That work is now covered by core database migration `packages/core/src/database/migration/20260510033149_session_usage.ts`, so there is no separate grexcode data-migration group.
...
- Existing schema ownership remains in `packages/core/src/**/*.sql.ts`; do not move table definitions back into `packages/grexcode`.

## Verification Commands

- `rg "@/storage/db|./storage/db|Database\.(use|transaction|effect|Client|getPath)|\bTxOrDb\b|\bTransaction\b" packages/grexcode/src`
- `bun typecheck` from `packages/grexcode`
- Relevant package tests from `packages/grexcode`, not the repo root
