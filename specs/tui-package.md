...
Move the canonical GrexCode terminal application from
`packages/grexcode/src/cli/cmd/tui` into a self-contained workspace package while
...
generic presentation libraries. It must not depend on `packages/grexcode`,
`packages/cli`, or `@grexlabs/core`.

The SDK is the TUI's GrexCode boundary. Missing backend data or operations must
...
- Do not preserve private imports by creating aliases from `packages/tui` back
  into `packages/grexcode`.
- Do not replace private `packages/grexcode` imports with `@grexlabs/core`
...
- Keep legacy CLI command parsing, server startup, worker management,
  authentication, and config discovery outside `@grexlabs/tui`.
...
- GrexCode domain data displayed by the TUI
...
```text
packages/grexcode/src/cli/cmd/tui
```

Its private dependency on `packages/grexcode` is primarily expressed through
the `@/*` TypeScript alias, which resolves to `packages/grexcode/src/*`.
...
Status: Completed. The private `@grexlabs/tui` workspace package now has an
...
- Add `packages/tui/package.json` with the name `@grexlabs/tui`.
...
- The package has no dependency on `grexcode`, `@grexlabs/cli`, or
  `@grexlabs/core`.
...
primitives now live in `@grexlabs/tui`. The legacy host consumes explicit
...
and build inputs through `@grexlabs/tui/runtime`. Movable app, component,
route, and feature-plugin code no longer reads GrexCode globals or process state;
...
Replace process-global GrexCode state with resolved TUI inputs.
...
`@grexlabs/tui/config` now owns schemas, defaults, keybind resolution, the
...
- Keep config path discovery, project/global precedence, migration, variable
  expansion, and plugin package installation in `packages/grexcode` initially.
...
canonical in `@grexlabs/tui`. Configured references resolve through the new
...
adapters rather than GrexCode domain implementations.

Make the SDK the only GrexCode domain boundary used by the TUI.
...
- Domain-facing TUI code imports GrexCode data and operations only from
  `@grexlabs/sdk`.
...
`@grexlabs/tui`. The legacy host injects a narrow plugin host that retains
...
- Move plugin rendering tests to `packages/tui`; retain installation/loading
  integration tests in `packages/grexcode`.
...
- The package has no imports from `packages/grexcode`, `packages/cli`, or
  `@grexlabs/core`.
...
public `@grexlabs/tui` root while retaining worker/server/config/plugin and
...
- Keep the legacy yargs commands corresponding to current `thread.ts` and
  `attach.ts` in `packages/grexcode`.
- Keep the legacy embedded worker and server startup in `packages/grexcode`.
- Change those adapters to load config, create transport inputs, and call the
  public `@grexlabs/tui` API.
...
- Remove obsolete `@tui/*` path mappings from `packages/grexcode`.
- Remove stale test fixtures and update all imports to package exports.
- Narrow `@grexlabs/tui` exports to intentional public entrypoints.
...
- No source under `packages/tui` imports `@/...`, `@grexlabs/core`, or either
  executable package.
...
- TUI package imports do not reach into executable or backend implementation
  packages.
- SDK wire data is treated as the source of truth for GrexCode domain state.
...
cd packages/grexcode && bun typecheck
...
rg '@grexlabs/core|packages/grexcode|packages/cli' packages/tui
rg 'src/cli/cmd/tui|@tui/' packages/grexcode/src packages/grexcode/test
```
...
- Build the current-platform `packages/grexcode` binary.
