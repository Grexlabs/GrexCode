...
The grexcode server has moved to the Effect HttpApi backend. Remaining work is
...
- remove the public in-memory `@grexlabs/llm` tool loop after replacing its
...
- plugins get global "grexcode" instance like in that post i showed
- grexcode instance has stuff like `grexcode.session.prompt()` or
  `grexcode.tool.register({...})`
...
- decide whether to preserve deprecated `@grexlabs/llm` orchestration exports
