...
Both `action` and `resource` use grexcode's existing wildcard matching behavior.
...
The relative policy precedence of direct project files and `.grexcode` files is intentionally deferred until `.grexcode` configuration is reviewed.
...
Plugins must not be allowed to add, remove, or override policy statements. Plugins can contribute functionality or configured providers; policy determines whether grexcode permits an operation through its managed execution paths.
