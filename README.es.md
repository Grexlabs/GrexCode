...
npm i -g grexcode-ai@latest        # o bun/pnpm/yarn
scoop install grexcode             # Windows
choco install grexcode             # Windows
brew install grexlabs/tap/grexcode # macOS y Linux (recomendado, siempre al día)
brew install grexcode              # macOS y Linux (fórmula oficial de brew, se actualiza menos)
sudo pacman -S grexcode            # Arch Linux (Stable)
paru -S grexcode-bin               # Arch Linux (Latest from AUR)
mise use -g grexcode               # cualquier sistema
nix run nixpkgs#grexcode           # o github:grexlabs/grexcode para la rama dev más reciente
```
...
GrexCode también está disponible como aplicación de escritorio. Descárgala directamente desde la [página de releases](https://github.com/grexlabs/grexcode/releases) o desde [grexlabs.in/download](https://grexlabs.in/download).
...
4. `$HOME/.grexcode/bin` - Alternativa por defecto
...
GrexCode incluye dos agentes integrados que puedes alternar con la tecla `Tab`.
...
Más información sobre [agentes](https://grexlabs.in/docs/agents).

### Documentación

Para más información sobre cómo configurar GrexCode, [**ve a nuestra documentación**](https://grexlabs.in/docs).

### Contribuir

Si te interesa contribuir a GrexCode, lee nuestras [docs de contribución](./CONTRIBUTING.md) antes de enviar un pull request.

### Proyectos basados en GrexCode

Si estás trabajando en un proyecto basado en GrexCode y usas "grexcode" como parte del nombre, por ejemplo, "grexcode-dashboard" u "grexcode-mobile", agrega una nota en tu README para aclarar que no está hecho por el equipo de GrexCode y que no está afiliado con nosotros de ninguna manera.

---

**Únete a nuestra comunidad** [Discord](https://discord.gg/grexcode) | [X.com](https://x.com/grexcode)
