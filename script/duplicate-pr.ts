...
import { createOpencode } from "@grexlabs/sdk"
...
`)
  ...
  const grexcode = await createOpencode({ port: 0 })
  ...
  try {
    ...
    const session = await grexcode.client.session.create()
    const result = await grexcode.client.session
      .prompt({
